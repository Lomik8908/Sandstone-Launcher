...
namespace Sandstone_Launcher
{
    ...
    {
        static public string AppVersionString = "1.0.0 RC-1";
        static public int AppVersion = 1;
        ...
        static void Main()
        {
            ...
            if (File.Exists("sl_settings.json")) try
                {
                    settings = JsonSerializer.Deserialize<Settings>(File.ReadAllText("sl_settings.json"));
                    saveSetting = true;
                }
                catch (Exception ex)
                {
        ...
        static public void LoadInstances()
        {
            ...
            if (File.Exists(InstancesPath)) try
                {
                    List<Instance> LoadUsers = JsonSerializer.Deserialize<List<Instance>>(File.ReadAllText(InstancesPath));
        ...
        static public void LoadUsers() {
            ...
            if (File.Exists(UsersPath)) try
                {
                    List<User> LoadUsers = JsonSerializer.Deserialize<List<User>>(File.ReadAllText(UsersPath));
        ...
        static void LoadAll()
        {
            ...
            if (!string.IsNullOrWhiteSpace(settings.gamedir))
                LauncherLib.GameDir = settings.gamedir;
            ...
            homeWindow.jre_check.Checked = settings.custom_java;
        ...
        static public void SaveInstances()
        {
            ...
            string InstancesPath = Path.Combine(LauncherLib.GameDir, "sl_instances.json");
            try { File.WriteAllText(InstancesPath, JsonSerializer.Serialize(Instances)); }
        ...
        static public void SaveUsers()
        {
            if (!saveUser) return;
            string UsersPath = Path.Combine(LauncherLib.GameDir, "sl_users.json");
            try { File.WriteAllText(UsersPath, JsonSerializer.Serialize(Users)); }
        ...
        static public void SaveSettings()
        {
            ...
            settings.custom_java = homeWindow.jre_check.Checked;
            ...
            try { File.WriteAllText("sl_settings.json", JsonSerializer.Serialize(settings)); } catch (Exception ex) { Logger.Err($"Couldn't save settings: {ex.Message}"); }
        }
        ...
        static public void SetGameDir(string Path)
        {
            if (string.IsNullOrWhiteSpace(Path))
                LauncherLib.GameDir = LauncherLib.DefaultGameDir;
            else
                LauncherLib.GameDir = Path;
            homeWindow.gamedir_box.Text = LauncherLib.GameDir;
            LoadInstances();
            LoadUsers();
        }

        static void InvokeUI(Action action) => homeWindow.Invoke(action);
        static void EndLaunch() {
            ...
            if (!homeWindow.IsDisposed)
                homeWindow.Invoke(new Action(() => {
                    ...
                    homeWindow.info_text.Text = string.Empty;
                    homeWindow.Show();
                }));
        }
        static public void Launch()
        {
            ...
            {
                MessageBox.Show(Users.Count <= 0 ? Lang.cresel_acc ?? "Create and select an account!" : Lang.sel_acc ?? "Sselect an account!", "Sandstone Launcher", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            };
            if (!(homeWindow.instance_box.SelectedItem is Instance instance)) {
                MessageBox.Show(Instances.Count <= 0 ? Lang.cresel_inst ?? "Create and select an instance!" : Lang.sel_inst ?? "Sselect an instance!", "Sandstone Launcher", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            };
            Launching = true;
            ...
            bool custJre = homeWindow.jre_check.Checked;
            ...
            string GameDir = instance.gamedir ?? (string.IsNullOrWhiteSpace(homeWindow.gamedir_box.Text) ? LauncherLib.GameDir : homeWindow.gamedir_box.Text);
            ...
            Task.Run(() => {
                ...
                if (mfJson["inheritsFrom"] != null)
                {
                    ...
                    if (ManifestTo != null)
                    {
                        ...
                        try { File.WriteAllText(verJson, JsonSerializer.Serialize(ManifestTo)); }
                ...
                if (user.usertype == "msa")
                    connected = WebHelper.IsAvailable("microsoft.com");
                ...
                if (JavaExec == null || !File.Exists(JavaExec) || !Launching) { EndLaunch(); return; }

                string ALJPath = Path.Combine(LauncherLib.GameDir, "libraries", "authlib-injector.jar");
                if (useInject && connected && user.usertype == "ely")
                    Accounts.DownloadAuthLibInjector(ALJPath, reCli, checkHash);
                if (!Launching) { EndLaunch(); return; }
                string ClassPath = LauncherLib.DownloadClasses(mfJson, gameVersion, reCli, checkHash);
                InvokeUI(() => { homeWindow.updateclient_box.Checked = false; });
                if (!Launching) { EndLaunch(); return; }

                if (checkAsset || reAsset)
                {
                    LauncherLib.DownloadAssets(mfJson, GameDir, reAsset, checkHash);
                ...
                if (gc?.arguments != null)
                    JvmAdditional.AddRange(gc.arguments);

                if (!string.IsNullOrWhiteSpace(jvmArgs))
                    JvmAdditional.AddRange(ArgSplitter.Split(jvmArgs));

                if (!string.IsNullOrWhiteSpace(instance.java_args))
                    JvmAdditional.AddRange(ArgSplitter.Split(instance.java_args));

                if (useInject && connected && user.usertype == "ely" && File.Exists(ALJPath))
                    JvmAdditional.AddRange(new string[] { $"-javaagent:{ALJPath}=ely.by", "-Dauthlibinjector.noShowServerName" });
                ...
                if (printArgs)
                    Logger.Log($"{JavaExec} {startInfo.Arguments}");
    ...
    class Settings
    {
        ...
        public bool custom_java { get; set; }
